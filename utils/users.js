/**
 * utils/users.js
 * JSON-backed authorized users store.
 *
 * users.json format:
 * { "users": [ { id, type, freeUsed, premiumUsed, freeLimit, premiumLimit } ] }
 */
const fs = require('fs');
...
function readUsers() {
  try {
    ...
    console.error('[users] Failed to read users.json:', e.message);
    return { users: [] };
  }
}
...
function findUser(userId) {
  ...
  return data.users.find((u) => String(u.id) === String(userId)) || null;
}
